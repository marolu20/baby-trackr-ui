...
import { formatDateWithWeekday, formatDateWithHoursMinutes } from "../../utils/DateUtils.js"
...
export function BabyEvents({babyId, onEdit, onDelete}) {
    ...
    useEffect(() => {
        window.addEventListener("refreshDashboardData", loadEvents);
        return () => {
            window.removeEventListener("refreshDashboardData", loadEvents);
        };
    }, [loadEvents]);
    ...
    function toTitleCase(str) {
        ...
        return str
            .toLowerCase()
            .replace(/_/g, " ") // Converts underscores like "WET_DIRTY" to spaces
            .split(" ")
    ...
    function getEventDetail(event) {
        ...
        switch (type) {
            ...
            case "SLEEP":
                return `${payload.sleepDuration} Minutes`;
            case "DIAPER":
                        ...
                        {sortedEvents.map((event) => (
                            <TableRow key={event.id} className="bg-white transition-colors duration-150 hover:bg-gray-50/50 dark:bg-gray-900 dark:hover:bg-gray-800/50" >
                                ...
                                <TableCell className="whitespace-nowrap px-6 py-4 text-gray-500 dark:text-gray-400">
                                    {formatDateWithHoursMinutes(event.payload?.eventTime || event.payload?.startTime)}
                                </TableCell>
